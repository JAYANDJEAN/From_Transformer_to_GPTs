...
from utils import BilingualDataset, causal_mask, get_default_config, get_weights_file_path, \
    get_latest_weights_file_path, ModelConfig


def greedy_decode(model: nn.Module, source: torch.Tensor, source_mask: torch.Tensor, tokenizer_src: Tokenizer,
                  tokenizer_tgt: Tokenizer, max_len: int, device: torch.device):
...
def run_validation(model: nn.Module, validation_ds: DataLoader, tokenizer_src: Tokenizer, tokenizer_tgt: Tokenizer,
                   max_len: int, device: torch.device, print_msg: callable, global_step: int, num_examples: int = 2):
    model.eval()
    count = 0
...
def train_model(config: ModelConfig):
    ...
    model = TransformerTorch(num_encoder_layers=3,
                             ...
                             tgt_vocab_size=tokenizer_tgt.get_vocab_size()).to(device)
